...
describe Enquo::Field do
	...
	describe "#encrypt_date" do
		context "with recent date" do
			let(:value) { Date.new(1970, 1, 1) }
			let(:context) { "test" }
			let(:result) { field.encrypt_date(value, context) }
			let(:json) { JSON.parse(result, symbolize_names: true) }
			let(:v1) { json[:v1] }
	...
	describe "#decrypt_date" do
		context "with a recent date" do
			...
			let(:plaintext) { field.decrypt_date(ciphertext, context) }
		...
		context "with a far-past date" do
			let(:value) { Date.new(-4000, 1, 1) }
			let(:context) { "test" }
			let(:ciphertext) { field.encrypt_date(value, context) }
			let(:plaintext) { field.decrypt_date(ciphertext, context) }
